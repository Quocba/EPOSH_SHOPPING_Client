...
import './Seller.UnpaidTransactionStyle.scss'
import { useSelector } from 'react-redux'
import { filterTransWithUnpaidList, filterTransWithUnpaidNumber } from '../../../../../utils/sellerHelper'

const SellerUnpaidTransaction: React.FC<{}> = () => {
    ...
    const [listTransactionUnpaid, setlistTransactionUnpaid] = useState([]);
    const isUnpaidPage = true;
    ...
    const totalPageTransactions = Math.ceil(filterTransWithUnpaidList?.length / transactionsPerPage);
    ...
    const init = async () => {
        let temp = filterTransWithUnpaidList(listTransaction)
        setlistTransactionUnpaid(temp)
    }
    ...
    useEffect(() => { }, [currentPage, listTransaction, listTransactionUnpaid, isUnpaidPage]);
    ...
    return (
        <>
            <Typography
                ...
                {filterTransWithUnpaidNumber(listTransaction)} Orders
            </Typography>
            <Box className='seller-transaction__cancel-list-container'>
                ...
                <Box className='seller-transaction__cancel-list-bottom'>
                    {listTransactionUnpaid?.map((data: any, index: any) => (
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginLeft: "30px", marginRight: "30px", paddingBottom: "10px" }}>
                    <Grid item xs={6}>
                        <label>
                            <b>
                    ...
                    <Stack>
                        {filterTransWithUnpaidNumber(listTransaction) > 1 && (
                            <Pagination
                                color="standard"
                                shape="rounded"
                                defaultPage={1}
...
export default SellerUnpaidTransaction
